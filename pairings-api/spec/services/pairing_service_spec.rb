...
RSpec.describe PairingService do
  def expected_item_shape
    {
      name: match(/\A\S.*\S\z/), description: be_present,
      category: be_in(Item::CATEGORIES.keys), subcategory: be_present,
      flavor_profiles: array_including(be_present),
      primary_flavor_profile: be_present, price_range: be_in(Item::PRICE_RANGES.keys)
    }
  end
  ...
  describe '#call' do
    ...
    context 'with valid response' do
      it 'returns a successful pairing object', :aggregate_failures,
         vcr: { cassette_name: 'pairing_service/valid_response' } do
        result = described_class.call(blob_image_id: blob.id)

        expect(result[:success?]).to be true
        expect(result[:payload]).to match(
          item1: expected_item_shape, item2: expected_item_shape,
          image_url: be_a(String).and(match(%r{\Ahttps?://})),
          pairing: { ai_reasoning: be_present, confidence_score: be_a(Float).and(be_between(0, 1)),
                     pairing_notes: be_a(String), strength: be_a(Integer).and(be_between(1, 5)) }
        )
      end
    end
